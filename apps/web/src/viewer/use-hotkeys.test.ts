...
describe('resolveHotkey', () => {
  ...
  it('цифры 1–4 выбирают режимы камеры в порядке ТЗ', () => {
    ...
    expect(resolveHotkey(press({ code: 'Digit2', key: '2' }))).toEqual({ kind: 'cameraMode', mode: 'free' });
    expect(resolveHotkey(press({ code: 'Digit3', key: '3' }))).toEqual({ kind: 'cameraMode', mode: 'cockpit' });
    expect(resolveHotkey(press({ code: 'Digit4', key: '4' }))).toEqual({ kind: 'cameraMode', mode: 'top' });
  });

  it('прочие клавиши не значат ничего', () => {
    // Ноль и пятёрка вне диапазона режимов, буквы и модификаторы — тоже мимо.
    expect(resolveHotkey(press({ code: 'Digit0', key: '0' }))).toBeNull();
    expect(resolveHotkey(press({ code: 'Digit5', key: '5' }))).toBeNull();
