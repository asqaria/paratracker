...
const app = buildApp({
  ...
  ...(auth
    ? {
        ...
        flightSettings: {
          setGlider: (args) => setFlightGlider(database.db, args),
          setPrivacy: (args) => setFlightPrivacy(database.db, args),
          ensureShareToken: (args) => ensureShareToken(database.db, args),
          resetShareToken: (args) => resetShareToken(database.db, args),
          findShared: (token) => findSharedFlight(database.db, token),
        },
        reviews: {
